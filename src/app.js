...
function appView() {
  ...
  return `<div class="app"><aside class="sidebar"><div class="brand">SIAWorkTable<small>由新哲文院算法社开发</small></div><nav class="nav">${navigation}</nav><div class="account">${esc(state.user)}<br><span data-sync>${state.session ? "已连接云端" : "离线模式"}</span></div></aside><main class="main">${content}</main><nav class="mobile-nav">${navigation}</nav>${modal ? modalView() : ""}</div>`;
}
